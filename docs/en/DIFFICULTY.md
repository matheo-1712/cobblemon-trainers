...
- **Battle gimmicks** (Mega Evolution, Z-Move, Dynamax, Terastallization). They do not depend on
  `difficulty` at all: the pack declares them, the AI does not have a bright idea. See
  [GIMMICKS.md](GIMMICKS.md). The three that wait for their moment still read better at
  `difficulty: 5` - their offensive trigger looks at the move already chosen, and a trainer that
  chooses badly is rarely holding the right one.
